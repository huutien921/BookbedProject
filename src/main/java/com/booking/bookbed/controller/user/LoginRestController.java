...
public class LoginRestController {
   ...
   private String regexUsername ="^[a-zA-Z0-9._-]{3,32}$";
    ...
    public ResponseEntity<Object> register(HttpServletRequest request, @RequestBody @Valid AccountRegister accountRegister,
       ...
       if (accountResult != null) {
        ...
        emailHelper.sendMailToken(tokenString, Utils.getBaseURL(request),"template_confirmToken.html", tokenResult.getAccount().getEmail());
    ...
    public ResponseEntity<Object> checkEmailRecovery(HttpServletRequest request,@RequestParam("email") String email) {
        try {
            ...
            if( account !=null){
                ...
                verificationToken.setExpiryDate(expireDate);
                VerificationToken tokenResult = verificationTokenService.save(verificationToken);
                emailHelper.sendMailRecovery(tokenString, Utils.getBaseURL(request),"template_confirmRecovery.html", tokenResult.getAccount().getEmail());
     ...
     public ResponseEntity<Object> recovery(HttpServletRequest request, @RequestBody @Valid PasswordRecovery passwordRecovery,
             BindingResult bindingResult, HttpSession httpSession) {
