...
/*	@Query("select new com.online.booking.entities.UserGroup(count(Account.id) as sumQuantity,"+
	...
	@Query("select new com.booking.bookbed.entities.UserGroup(count(a.id) as sumQuantity,month(a.created) as year ) from Account as a join a.roleAccounts ra join ra.role r where r.id=:id and year(a.created)='2019'  group by month(a.created)")
			public List<UserGroup> statisticalUserses(@Param("id") Integer id);
